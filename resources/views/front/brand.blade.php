    ...
    <div class="px-4">
        <div class="flex items-center justify-between w-full rounded-2xl overflow-hidden bg-white">
            <div class="flex flex-col gap-[2px] px-[30px] pr-4">
                ...
                <p class="text-[#878785]"> {{ $brand->shoes->count() }} Products</p>
            </div>
            <div class="flex shrink-0 w-[140px] h-[120px] overflow-hidden">
                <img src="{{ Storage::url($brand->logo) }}" class="w-full h-full object-contain" alt="{{ $brand->name }}">
            </div>
        </div>
    </div>

    <section id="brand-products" class="flex flex-col gap-4 px-4 mb-[111px]">
        <div class="flex items-center justify-between">
            <h2 class="font-bold leading-[20px]">Latest from <br>{{ $brand->name }}</h2>
        ...
        <div class="flex flex-col gap-4">
            ...
            <a href="{{ route('front.details', $shoe->slug) }}">
                <div class="flex items-center rounded-3xl p-[10px_16px_16px_10px] gap-[14px] bg-white transition-all duration-300 hover:ring-2 hover:ring-[#FFC700]">
                    <div class="w-20 h-20 flex shrink-0 rounded-2xl bg-[#D9D9D9] overflow-hidden">
                        <img src="{{ Storage::url($shoe->thumbnail) }}" class="w-full h-full object-cover" alt="{{ $shoe->name }}">
                    </div>
            ...
            @empty
            <p>Uppps, tidak ada produk tersedia!</p>
            @endforelse
        </div>
    </section>
</x-layouts.app>
